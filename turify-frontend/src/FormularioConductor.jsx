...
const DropZone = ({ label, name, onChange, file }) => {
  ...
  return (
    <div
      ...
      style={{ border: `2px dashed ${isDragging ? BRAND_GREEN : 'var(--t-linea)'}`, borderRadius: '12px', padding: '20px', textAlign: 'center', backgroundColor: isDragging ? 'var(--t-musgo)' : 'var(--t-niebla)', cursor: 'pointer', transition: 'all 0.2s ease', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '100px' }}
    >
      ...
      {file ? (
        <div style={{ color: BRAND_GREEN, fontWeight: '600' }}>✅ Archivo cargado:<br /><span style={{ fontSize: '12px', color: 'var(--t-tinta)' }}>{file.name}</span></div>
      ) : (
        <div style={{ color: 'var(--t-piedra)' }}>
          <span style={{ fontSize: '28px', display: 'block', marginBottom: '8px' }}>📁</span>
          <strong style={{ color: 'var(--t-tinta)', fontSize: '14px' }}>{label}</strong><br />
...
const VistaEstadoDocumentos = ({ estadoData, onVolver }) => {
  const config = {
    PENDIENTE: { icon: '⏳', color: 'var(--t-chiva-texto)', bg: 'var(--t-chiva-suave)', titulo: 'Documentos en revisión', descripcion: 'Tus documentos están siendo revisados por el administrador. Te notificaremos cuando haya una respuesta.' },
    APROBADO:  { icon: '✅', color: 'var(--t-musgo-texto)', bg: 'var(--t-musgo)', titulo: '¡Eres conductor!', descripcion: 'Todos tus documentos han sido aprobados. Ya puedes recibir solicitudes de viaje.' },
    RECHAZADO: { icon: '❌', color: 'var(--t-alerta-texto)', bg: 'var(--t-alerta-suave)', titulo: 'Documentos rechazados', descripcion: 'Algunos documentos fueron rechazados. Corrígelos y vuelve a enviarlos.' },
  };
  const c = config[estadoData.estado] || config.PENDIENTE;

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      style={{ maxWidth: '600px', margin: '0 auto', backgroundColor: 'var(--t-papel)', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '40px 30px' }}>
        ...
        <div style={{ backgroundColor: c.bg, border: `1px solid ${c.color}33`, borderRadius: '12px', padding: '24px', textAlign: 'center', marginBottom: '24px' }}>
          ...
          <p style={{ margin: 0, color: 'var(--t-piedra)', fontSize: '14px', lineHeight: '1.6' }}>{c.descripcion}</p>
        </div>

        {/* Lista de documentos con estado */}
        {estadoData.documentos && (
          <div>
            <p style={{ margin: '0 0 12px', fontWeight: '700', fontSize: '13px', color: 'var(--t-piedra)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Estado por documento</p>
            {estadoData.documentos.map((doc, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', borderRadius: '8px', border: '1px solid var(--t-linea)', marginBottom: '8px', backgroundColor: 'var(--t-niebla)' }}>
                <span style={{ fontSize: '13px', color: 'var(--t-tinta)', fontWeight: '600' }}>{ETIQUETA_DOCUMENTO[doc.tipo] || doc.tipo}</span>
                <span style={{
                  padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700',
                  backgroundColor: doc.estado === 'APPROVED' ? 'var(--t-musgo)' : doc.estado === 'REJECTED' ? 'var(--t-alerta-suave)' : 'var(--t-chiva-suave)',
                  color: doc.estado === 'APPROVED' ? 'var(--t-musgo-texto)' : doc.estado === 'REJECTED' ? 'var(--t-alerta-texto)' : 'var(--t-chiva-texto)'
                }}>
        ...
        {estadoData.documentos_rechazados && estadoData.documentos_rechazados.length > 0 && (
          <div style={{ backgroundColor: 'var(--t-alerta-suave)', border: '1px solid var(--t-alerta-linea)', borderRadius: '10px', padding: '16px', marginTop: '16px' }}>
            <p style={{ margin: '0 0 8px', fontWeight: '700', color: 'var(--t-alerta-texto)', fontSize: '13px' }}>📋 Documentos que debes volver a enviar:</p>
...
const FormularioConductor = () => {
  ...
  const inputBloqueadoStyle = { padding: '14px', border: '1px solid var(--t-linea)', borderRadius: '8px', width: '100%', boxSizing: 'border-box', backgroundColor: 'var(--t-linea)', color: 'var(--t-piedra)', cursor: 'not-allowed' };

  // --- RENDER ---
  if (estadoCarga === 'cargando') {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--t-niebla-2)', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        <div style={{ textAlign: 'center', color: 'var(--t-piedra)' }}>
  ...
  if (estadoCarga === 'mostrar_estado') {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: 'var(--t-niebla-2)', padding: '40px 20px', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--t-niebla-2)', padding: '40px 20px', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        style={{ maxWidth: '850px', margin: '0 auto', backgroundColor: 'var(--t-papel)', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        ...
        {estadoData?.estado === 'RECHAZADO' && (
          <div style={{ backgroundColor: 'var(--t-alerta-suave)', border: '1px solid var(--t-alerta-linea)', margin: '20px 30px 0', borderRadius: '10px', padding: '14px 18px' }}>
            <p style={{ margin: 0, fontWeight: '700', color: 'var(--t-alerta-texto)', fontSize: '13px' }}>
        ...
        <form onSubmit={enviarFormularioConductor} style={{ padding: '40px 30px' }}>
          ...
          <h3 style={{ borderBottom: '2px solid var(--t-niebla-2)', paddingBottom: '10px', color: 'var(--t-tinta)', marginTop: 0 }}>👤 1. Datos Personales</h3>
          <p style={{ fontSize: '13px', color: 'var(--t-piedra)', marginBottom: '20px' }}>Tus datos básicos han sido cargados desde tu cuenta.</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '40px' }}>
            ...
            <input type="number" name="age" placeholder="Edad" onChange={handleInputConductor} required min="18" style={{ padding: '14px', border: '1px solid var(--t-linea)', borderRadius: '8px', width: '100%', boxSizing: 'border-box' }} />
            <select name="affiliated_company" onChange={handleInputConductor} required style={{ padding: '14px', border: '1px solid var(--t-linea)', borderRadius: '8px', width: '100%', boxSizing: 'border-box', backgroundColor: 'var(--t-papel)' }}>
          ...
          <h3 style={{ borderBottom: '2px solid var(--t-niebla-2)', paddingBottom: '10px', color: 'var(--t-tinta)' }}>🚗 2. Datos del Vehículo</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '40px' }}>
            <input type="text" name="plate" placeholder="Placa (Ej. ABC-123)" onChange={handleInputConductor} required style={{ padding: '14px', border: '1px solid var(--t-linea)', borderRadius: '8px', width: '100%', boxSizing: 'border-box', textTransform: 'uppercase' }} />
            <input type="number" name="capacity" placeholder="Capacidad (asientos)" onChange={handleInputConductor} required min="1" max="44" style={{ padding: '14px', border: '1px solid var(--t-linea)', borderRadius: '8px', width: '100%', boxSizing: 'border-box' }} />
            <div style={{ gridColumn: 'span 2' }}>
          ...
          <div style={{ marginBottom: '40px' }}>
            <p style={{ fontSize: '12px', fontWeight: '700', color: 'var(--t-tinta)', marginBottom: '4px' }}>Comodidades de tu vehículo (opcional)</p>
            <p style={{ fontSize: '12px', color: 'var(--t-piedra)', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {[
                ...
                <label key={campo} style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '13px', color: 'var(--t-tinta)', border: '1px solid var(--t-linea)', borderRadius: '8px', padding: '10px 12px', cursor: 'pointer' }}>
          ...
          <h3 style={{ borderBottom: '2px solid var(--t-niebla-2)', paddingBottom: '10px', color: 'var(--t-tinta)' }}>📄 3. Documentación Reglamentaria</h3>
          <p style={{ fontSize: '13px', color: 'var(--t-piedra)', marginBottom: '20px' }}>Adjunta los documentos en formato PDF o Imagen.</p>
          ...
          <button type="submit" disabled={enviando}
            style={{ width: '100%', background: enviando ? 'var(--t-piedra-clara)' : BRAND_GREEN, color: '#fff', border: 'none', padding: '16px', borderRadius: '8px', fontSize: '18px', fontWeight: 'bold', cursor: enviando ? 'not-allowed' : 'pointer', transition: 'background 0.3s' }}>
